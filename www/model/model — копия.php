...
function products($category, $order_db, $start_position, $perpage){
    $query = "(SELECT goods_id, name, img, anons, price, leader, new, sale, date 
                    FROM goods 
                        WHERE goods_brandid = $category AND visible='1')
                UNION
                (SELECT goods_id, name, img, anons, price, leader, new, sale, date
                    FROM goods 
                        WHERE goods_brandid IN
                    (  
                        SELECT brand_id FROM brands WHERE parent_id = $category
                        ) AND visible='1') ORDER BY $order_db LIMIT $start_position, $perpage";
...
function filter($category, $startprice, $endprice){
    ...
    if($category OR $endprice){ 
        ...
        if($category){
            ...
            $predicat2 = "UNION 
                            (SELECT goods_id, name, img, price, leader, new, sale
                            FROM goods
                                WHERE goods_brandid IN
                                (
...
function registration(){
    ...
    if (empty($name)) $error .= '<li>Не указаны Ф.И.О.!</li>';
...
function add_order(){ //принимать данные из формы, брабатывать их, проверять заполнины ли все обязательные данные
    ...
    save_order($customer_id, $delivery_id, $comment);  
}
...
function save_order($customer_id, $delivery_id, $comment){
 $comment = clear($comment);
 $query = "INSERT INTO `order` (customer_id, data, delivery_id, comment)
                VALUES ($customer_id, NOW(), $delivery_id, '$comment')";
...
function get_user1($user_id){
    $query = "SELECT customers.name, customers.email, customers.phone, customers.address, customers.login, customers.password
                FROM customers
...
function user_edit($user_id){
            ...
            if($key == "password"){
                $value = trim($value); //обрезаем лишние пробелы
                if(!empty($value)){
                    $value = md5($value);
                }else{
                    continue;
            ...
            $data[$key] = $value; //формируем массив значений введнных пользователем
        }
        ...
        if(mysql_affected_rows() > 0){
            ...
            if($user_id == $_SESSION['auth']['customer_id']){ //если мы изменили свое же имя, чтобы оно обновилось в углу страницы
